...
class LoadGenerationControllerTest extends FunSuite with MockitoSugar with ScalaFutures with Matchers with BeforeAndAfterAll {
  ...
  test("Accepts load generation") {
    ...
    val request = FakeRequest("POST", "/").withJsonBody(Json.parse("""{"weatherStation":{"id":"chris"},"loadSpec":{"startDate":"2015-04-03T10:55:00.000Z", "endDate":"2015-04-15T09:10:00.000Z", "duration": 2000}}"""))

    val result = call(underTest.generateLoad(), request)

    status(result) should be(200)
    val time: DateTimeFormatter = ISODateTimeFormat.dateTime()
    verify(loadGenerationService).generateLoad(WeatherStationId("chris"), LoadSpec(new DateTime(time.parseDateTime("2015-04-03T10:55:00.000Z")), time.parseDateTime("2015-04-15T09:10:00.000Z"), Duration.millis(2000)))
  }
